...
def main(rawargs=None):
    ...
    to_sensor_azimuth_lut_grid, to_sensor_zenith_lut_grid = get_metadata_from_obs(
    ...
    if not os.path.exists(paths.lbl_working_path) or not os.path.exists(paths.radiance_working_path):
    ...
    if opt["presolve_wv"]:
        ...
        if not os.path.exists(envi_header(paths.h2o_subs_path)) or not os.path.exists(paths.h2o_subs_path):
            ...
            if gip["filepaths"]["emulator_base"] is None:
                for to_rm in ["*r_k", "*t_k", "*tp7", "*wrn", "*psc", "*plt", "*7sc", "*acd"]:
                    cmd = "rm " + os.path.join(paths.lut_h2o_directory, to_rm)
                    logging.info(cmd)
                    os.system(cmd)
        else:
    ...
    if not os.path.exists(paths.uncert_subs_path) or not os.path.exists(paths.rfl_subs_path):
        write_modtran_template(
            gip=gip,
            fid=paths.fid,
            altitude_km=mean_altitude_km,
        ...
        if gip["filepaths"]["emulator_base"] is None:
            for to_rm in ["*r_k", "*t_k", "*tp7", "*wrn", "*psc", "*plt", "*7sc", "*acd"]:
                cmd = "rm " + os.path.join(paths.lut_modtran_directory, to_rm)
                logging.info(cmd)
                os.system(cmd)
    ...
    if not os.path.exists(paths.rfl_working_path) or not os.path.exists(paths.uncert_working_path):
        # Empirical line
